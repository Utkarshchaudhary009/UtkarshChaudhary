...
export async function GET(req: NextRequest) {
  try {
    ...
    if (searchParams.has("target.categories")) {
      filters["target.categories"] = { $in: [searchParams.get("target.categories")] };
    }
    ...
    if (searchParams.has("dateFrom") && searchParams.has("dateTo")) {
      ...
      filters.created_at = { $gte: new Date(searchParams.get("dateFrom") as string) };
    } else if (searchParams.has("dateTo")) {
      filters.created_at = { $lte: new Date(searchParams.get("dateTo") as string) };
    }
    ...
    if (searchParams.has("sortBy")) {
      ...
      if (["title", "impressions", "clicks", "created_at"].includes(sortBy as string)) {
